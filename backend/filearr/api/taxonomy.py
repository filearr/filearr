...
async def sync_seed(
    ...
    """Adopt extensions/groups the shipped SEED has gained, without disturbing any
    edit an operator has made. ADD-ONLY, and idempotent.

    Why this exists. :func:`filearr.taxonomy.load` reads the DB **only when a
    version row exists**; with no row it serves the code seed directly. So a
    deployment that has never touched the taxonomy picks up a widened seed for
    free on upgrade — and a deployment that edited it even once is frozen at the
    seed it was first written from, because the DB became authoritative. That is
    a silent failure: we ship a better catalogue, nothing changes, and there is
    no error to explain why. This endpoint is the bridge.

    What it will and will not do:

    * **Adds** an extension only when the DB maps it NOWHERE. An extension the
      operator deliberately reparented (``raw`` moved out of ``raw-photo``, say)
      is left exactly where they put it and reported under ``skipped`` — the seed
      is not treated as more authoritative than a human decision.
    * **Creates** a seed category/group that the DB lacks, so a genuinely new
      grouping can arrive. Existing categories/groups are never relabelled,
      re-parented or re-sorted; only their missing extensions are filled in.
    * **Never deletes or moves anything.** There is deliberately no "reset to
      seed" here; that would be a destructive operation wearing a maintenance
      operation's clothes.

    ``dry_run=true`` reports exactly what a real run would change and commits
    nothing — worth using first on a taxonomy with a lot of local edits.
