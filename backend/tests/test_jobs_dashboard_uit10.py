...
async def test_jobs_summary_composition(proc_connector, monkeypatch):
    ...
    assert set(summary) == {
        "agent_replication", "maintenance", "queues", "extract", "running",
        "failed_recent", "meili", "scans_running", "stalled", "priorities",
        "staged_pipeline", "disk", "resources", "thumbs", "scan_throughput",
        "upcoming",
    }
    # Feature off in this harness -> the replication activity list is empty.
    assert summary["agent_replication"] == []
    # Fresh DB (post-migration seed row) -> maintenance mode inactive.
    assert summary["maintenance"] == {
        "active": False, "reason": None, "started_at": None,
    }
