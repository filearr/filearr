...
async def test_mint_list_use_revoke(client):  # noqa: F811
    ...
    # unknown scope -> 422
    r = await c.post("/api/v1/api-keys", json={"name": "x", "scopes": ["root"]})
...
async def test_admin_scoped_key_and_non_admin_refused(client):  # noqa: F811
    c, maker, _ = client
    await _bootstrap_admin(c)
    r = await c.post("/api/v1/api-keys", json={"name": "ops", "scopes": ["admin"]})
    assert r.status_code == 201, r.text
    key = r.json()["key"]
    hdr = {"Authorization": f"Bearer {key}"}
    # admin implies everything, including minting further keys
    r = await c.post("/api/v1/api-keys", json={"name": "child", "scopes": ["read"]}, headers=hdr)
    ...
    assert (await c.post("/api/v1/api-keys", json={"name": "n"}, headers=ch)).status_code == 403
