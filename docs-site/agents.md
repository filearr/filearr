...
| `permissions` | The **full normalized permission record** per entry (owner, group, every allow/deny ACE with native mask, inheritance flags, fidelity stamp). Linux: mode bits + POSIX ACL xattrs, pure Go; cifs-without-`cifsacl` mounts stamped `synthesized_from_mode`. Windows: owner + full DACL via the security descriptor (local and UNC), **plus the ACL of every local SMB share covering the path** (agent ≥ 1.5.4; `Get-SmbShareAccess`, one call per share per run, entries tagged `source: share`) so central can reconcile share vs file permissions — see [Security → Share permissions vs file permissions](security.md#share-vs-file-permissions). macOS (agent ≥ 1.5.3): mode bits + the extended ACL via a bounded `ls -led` read (named permissions → verbs, order preserved; BSD file flags like `uchg`/`schg` as a separate `flags` list; a TCC/Full-Disk-Access denial surfaces as the entry's collector error, never a silent empty ACL). Central stores each record in `permission_snapshots` (unchanged re-collections write nothing; newest *N* per path, `FILEARR_PERMISSION_SNAPSHOTS_RETAIN`, default 10), diffs each new snapshot against the previous one (the `permission_changes` report + the **System: permission change** alert rule) and the Reports page gains the `permissions_*` reports. | Linux, macOS, Windows | medium |
...
!!! note "How the inventory settings are enforced (since 2026-08-20)"
    `log_level` is applied **by the agent** (≥ 1.5.3): the daemon live-retunes
    ...
    forward-looking knob (central's drift pipeline does not need it).
