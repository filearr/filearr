...
| `permissions_share_vs_fs` | Files reachable over an **SMB share** whose share-level ACL and file-level ACL **disagree on who can read**. Access through a share is the *intersection* of both (Windows' Effective Access: "Access limited by: Share, File Permissions"), so a share granting Everyone Read over files that only name Authenticated Users does **not** expose them anonymously — but either direction is worth a look: a share wider than its files is a dead grant one file-ACL slip away from being live; files wider than their share are protected only by the share. Per-layer read tier, the effective tier through the share, and a plain-language note. Needs the Windows agent's share-ACL read (agent ≥ 1.5.4). |
...
| `permissions_explicit_outliers` | The by-principal list minus entries that merely **restate the parent directory's** ACE for the same principal/type/verbs (inheritance re-applied by robocopy `/SEC`, `icacls` reset, `cp -p`…). What remains is the hand-granted **deviation** an audit cares about. `baseline` is `deviates` when the parent path has a snapshot to compare against, `unknown` when it does not (inventory the parent directory too for a definitive verdict). macOS reads land with agent ≥ 1.5.3 (named ACL entries + BSD flags via `ls -led`). |
